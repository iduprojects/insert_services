...
class Properties:
    ...
    def copy(self) -> 'Properties':
        return Properties(self.db_addr, self.db_port, self.db_name, self.db_user, self.db_pass, connect_timeout=self.connect_timeout)

    @property
    def conn_string(self) -> str:
        return f'host={self.db_addr} port={self.db_port} dbname={self.db_name}' \
                f' user={self.db_user} password={self.db_pass} application_name=insert_services connect_timeout={self.connect_timeout}'
    @property
