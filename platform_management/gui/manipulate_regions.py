...
class PlatformRegionsTableWidget(ColoringTableWidget):
    ...
    def __init__(self, services: Sequence[Sequence[Any]], changed_callback: Callable[[int, str, Any, Any, bool], None],
        ...
        super().__init__(services,
                ...
                list(map(lambda x: x[0], filter(lambda y: y[1] in ('id', '-'), enumerate(PlatformRegionsTableWidget.LABELS_DB)))),
                parent=parent)
...
class RegionsWindow(QtWidgets.QWidget):
    ...
    def __init__(self, db_properties: Properties, on_close: Optional[Callable[[], None]] = None, parent: Optional[QtWidgets.QWidget] = None):
        ...
        self._edit_buttons = RegionsWindow.EditButtons(QtWidgets.QPushButton('Добавить регион'),
                QtWidgets.QPushButton('Изменить регион'), QtWidgets.QPushButton('Удалить регион'),
                QtWidgets.QPushButton('Посмотреть геометрию'), QtWidgets.QPushButton('Сохранить изменения в БД'),
    ...
    def _on_regions_load(self) -> None:
        ...
        with self._db_properties.conn.cursor() as cur:
            cur.execute('SELECT id, name, code, ST_Y(center), ST_X(center), ST_GeometryType(geometry),'
    ...
    def _on_cell_change(self, row: int, column_name: str, old_value: Any, new_value: Any, is_valid: bool) -> None:
        ...
        if is_valid:
            self._log_window.insertHtml(f'<font color=yellowgreen>Изменен регион с id={city_id}. {column_name}:'
                    f' "{_to_str(old_value)}"->"{_to_str(new_value)}"</font><br>')
        else:
            self._log_window.insertHtml(f'<font color=#e6783c>Не изменен регион с id='
        ...
        with self._db_properties.conn.cursor() as cur:
            cur.execute(f"UPDATE regions SET {db_column} = %s, updated_at = date_trunc('second', now()) WHERE id = %s",
                    (new_value, city_id))
    ...
    def _on_region_add(self) -> None:
        ...
        if dialog.exec() != QtWidgets.QDialog.Accepted or self._additional_conn is None:
            ...
            if new_geom_tuple is None:
                self._log_window.insertHtml(f'<font color=#e6783c>Регион не добавлен, ошибка в геометрии</font><br>')
                return
            latitude, longitude, geom_type = new_geom_tuple
            with self._db_properties.conn.cursor() as cur:
                cur.execute('INSERT INTO regions (name, code, geometry, center) VALUES'
                        '   (%s, %s, ST_SetSRID(ST_GeomFromGeoJSON(%s), 4326), ST_SnapToGrid(ST_Centroid(ST_SetSRID(ST_GeomFromGeoJSON(%s), 4326)), 0.000001))'
                        ' RETURNING id',
                        (dialog.name(), dialog.code(),dialog.get_geometry(), dialog.get_geometry()))
                city_id = cur.fetchone()[0] # type: ignore
            self._log_window.insertHtml(f'<font color=yellowgreen>Добавлен регион "{dialog.name()}" c id={city_id}</font><br>')
    ...
    def _on_city_edit(self) -> None:
        ...
        city_id = self._table.item(row, 0).text()
        with self._db_properties.conn.cursor() as cur:
            cur.execute('SELECT ST_AsGeoJSON(geometry)::jsonb, name, code FROM cities WHERE id = %s', (self._table.item(row, 0).text(),))
            geometry, name, code = cur.fetchone() # type: ignore
            geometry = geometry
        dialog = RegionCreation(f'Внесение изменений в регион в строке под номером {row + 1}', json.dumps(geometry, indent=2), name, code)
        if dialog.exec() != QtWidgets.QDialog.Accepted or self._additional_conn is None:
            return
        else:
            new_geom_tuple = check_geometry_correctness(dialog.get_geometry(), self._additional_conn)
            if new_geom_tuple is None:
                self._log_window.insertHtml(f'<font color=#e6783c>Регион "{name}" с id={city_id} не изменен, ошибка в геометрии</font><br>')
                return
            ...
            if geometry != new_geometry:
                ...
                with self._db_properties.conn.cursor() as cur:
                    cur.execute('UPDATE regions SET geometry = ST_SetSRID(ST_GeomFromGeoJSON(%s), 4326),'
                ...
                self._log_window.insertHtml(f'<font color=yellowgreen>Изменена геометрия региона "{dialog.name()}" c id={city_id}:'
                        f' {self._table.item(row, 5).text()}({self._table.item(row, 3).text()}, {self._table.item(row, 4).text()})'
    ...
    def _on_city_delete(self) -> None:
        ...
        if len(rows) == 0:
            return
        if len(rows) > 1:
            is_deleting = QtWidgets.QMessageBox.question(self, 'Удаление регионов',
                    f'Вы уверены, что хотите удалить регионы в строках под номерами: {", ".join(map(str, rows))}?')
        else:
            object_row = next(iter(rows))
            is_deleting = QtWidgets.QMessageBox.question(self, 'Удаление региона', f'Вы уверены, что хотите удалить регионы в строке под номером {object_row}?')
