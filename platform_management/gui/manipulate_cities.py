...
def _to_str(int_value: Optional[Union[int, float, str]]) -> str:
    return str(int_value) if int_value is not None else ""
...
class CitiesWindow(QtWidgets.QWidget):
    ...
    def _on_city_add(self) -> None:
        ...
        self._table.setItem(row, 4, QtWidgets.QTableWidgetItem(_to_str(dialog.region())))
        self._table.setItem(row, 5, QtWidgets.QTableWidgetItem(_to_str(dialog.division_type())))
    ...
    def _on_municipality_edit(
        ...
        if len(changes) != 0:
            ...
            for what_changed, old_value, new_value in changes:
                self._log_window.insertHtml(
                    f'&nbsp;&nbsp;{what_changed}: "{_to_str(old_value)}"->"{_to_str(new_value)}"<br>'
                )
